...
$sheet->setCellValue('D1', 'ID Anggota');
$sheet->setCellValue('E1', 'Tanggal Peminjaman');
$sheet->setCellValue('F1', 'Tanggal Pengembalian');
 
$query = mysqli_query($db,"select * from tbtransaksi WHERE tglkembali IS NOT null");	//mengambil data table tbbuku dari db
...
while($row = mysqli_fetch_array($query))
{
	...
	$sheet->setCellValue('C'.$i, $row['idbuku']);
	$sheet->setCellValue('D'.$i, $row['idanggota']);	
	$sheet->setCellValue('E'.$i, $row['tglpinjam']);	
	$sheet->setCellValue('F'.$i, $row['tglkembali']);		
	$i++;
}
...
$filename = 'Data-Transaksi-Perpustakaan.xlsx';
